...
import { createTranscriber, GroqTranscriber, ParakeetTranscriber, TranscriberSwitch } from "./transcribe.ts";
...
test("switch swaps mode when the target's creds exist", () => {
  const sw = new TranscriberSwitch({ mode: "local", groqApiKey: "k", parakeetUrl: "http://p" });
...
test("switch refuses to move to a backend with no creds and keeps the old mode", () => {
  const sw = new TranscriberSwitch({ mode: "local", groqApiKey: "", parakeetUrl: "http://p" });
