...
import { parseHabits, completeHabitLine, parseHabitRef } from "./parse.ts";
...
export class HabitsCommand {
  ...
  async prompt(date: string, announceEmpty = false): Promise<void> {
    ...
    const pending = daily ? parseHabits(daily.content, config.obsidian.habitsHeading).filter((h) => !h.done) : [];
    if (!pending.length) {
      log.info({ date, hasNote: !!daily }, "no pending habits to review");
      if (announceEmpty) {
        await this.bot.api.sendMessage(
          config.telegram.allowedUserId,
          daily ? `✅ All habits already done for ${date}.` : `No habits found for ${date}.`,
        );
      }
      return;
    }
    log.info({ date, count: pending.length }, "starting habit review");
    await this.ask(date, 0, `🌱 Habits for ${date} — ${pending.length} to review:`);
  }
  ...
  private async ask(date: string, fromIndex: number, header = ""): Promise<void> {
    const daily = await this.obsidian.readDailyNote(date);
    if (!daily) {
      log.warn({ date }, "note vanished mid-review — stopping");
      return;
    }
    const habit = parseHabits(daily.content, config.obsidian.habitsHeading)
      .find((h) => h.index >= fromIndex && !h.done);
    if (!habit) {
      log.info({ date }, "habit review complete");
      await this.bot.api.sendMessage(config.telegram.allowedUserId, `✅ Habits reviewed for ${date}.`);
      return;
    }
    log.debug({ date, index: habit.index, kind: habit.field ? "value" : "yes/no" }, "asking habit");
    ...
    await this.bot.api.sendMessage(config.telegram.allowedUserId, `${lead}🌱 ${habit.label}?`, { reply_markup: kb });
  }

  /** Handle a Yes/No tap on a boolean habit, then advance to the next one. */
  async handleTap(ctx: any, date?: string, idxStr?: string, verd?: string): Promise<void> {
    ...
    const daily = await this.obsidian.readDailyNote(date);
    const habit = daily && parseHabits(daily.content, config.obsidian.habitsHeading).find((h) => h.index === index);
    if (!daily || !habit) {
    ...
    if (verd === "y") {
      ...
      await this.obsidian.writeNote(daily.path, daily.content.replace(habit.line, () => updated));
  ...
  async handleReply(ctx: any): Promise<void> {
    ...
    const habit = daily && parseHabits(daily.content, config.obsidian.habitsHeading).find((h) => h.index === ref.index);
    if (!daily || !habit) {
      log.warn({ date: ref.date, index: ref.index }, "habit value reply ignored: note or habit gone");
      return void ctx.reply("Couldn't find that habit to update.");
    }
    const updated = completeHabitLine(habit.line, ref.date, value);
    await this.obsidian.writeNote(daily.path, daily.content.replace(habit.line, () => updated));
    log.info({ date: ref.date, index: ref.index, label: habit.label }, "habit value recorded");
