...
import { msUntilNext, plainDate, previousDate } from "./time.ts";
...
export class Scheduler {
	...
	private scheduleSummary(): void {
		...
		log.debug(
			{ inMs: wait, at: config.summaryTime },
		...
		const t = setTimeout(async () => {
			try {
				...
				log.error({ err: e }, "summary failed");
			}
	...
	private scheduleRating(): void {
		...
		log.debug(
			{ inMs: wait, at: config.ratingTime },
			"next daily rating prompt scheduled",
		);
		const t = setTimeout(async () => {
			// Fires at 00:00 → the day that just ended is yesterday.
			try {
				await this.askRating(previousDate());
			} catch (e) {
				log.error({ err: e }, "rating prompt failed");
			}
	...
	private scheduleHabits(): void {
		...
		log.debug(
			{ inMs: wait, at: config.habitsTime },
			"next daily habit review scheduled",
		);
		const t = setTimeout(async () => {
			// Fires at 00:00 → review the day that just ended, i.e. yesterday.
			try {
				await this.askHabits(previousDate());
			} catch (e) {
				log.error({ err: e }, "habit prompt failed");
			}
