...
type Msg =
  | { type: "assistant"; message: { content: Array<{ type: string; text?: string }>; usage?: { input_tokens?: number; output_tokens?: number } } }
...
const assistantText = (text: string, usage?: { input_tokens?: number; output_tokens?: number }): Msg => ({
  type: "assistant",
  message: { content: [{ type: "text", text }], usage },
});

test("enrich returns parsed text, ambiguous, and usage from clean JSON", async () => {
  const body = JSON.stringify({ text: "Ran with [[John]] today", ambiguous: [{ surface: "no", note: "Norway" }] });
  const { fn } = fakeQuery([assistantText(body, { input_tokens: 10, output_tokens: 3 })]);
  const out = await new Enricher(undefined, fn).enrich({ text: "Ran with John today", candidates: [{ surface: "John", note: "John" }] });
...
test("enrich throws when the response has no usable JSON", async () => {
  ...
  await assert.rejects(new Enricher(undefined, fn).enrich({ text: "x", candidates: [] }), /no usable JSON/);
});

test("enrich lists candidates in the prompt, or '(none)' when empty", async () => {
  const withCands = fakeQuery([assistantText('{"text":"ok"}')]);
  await new Enricher(undefined, withCands.fn).enrich({ text: "hey", candidates: [{ surface: "John", note: "John Doe" }] });
  assert.match(withCands.calls[0]!.prompt as string, /- "John" -> \[\[John Doe\]\]/);

  const noCands = fakeQuery([assistantText('{"text":"ok"}')]);
  await new Enricher(undefined, noCands.fn).enrich({ text: "hey", candidates: [] });
  assert.match(noCands.calls[0]!.prompt as string, /Candidate links:\n\(none\)/);
});
...
test("enrich strips the triple-quote fence from user text so it can't break out", async () => {
  ...
  await new Enricher(undefined, fn).enrich({ text: 'say """hi""" now', candidates: [] });
...
test("run falls back to the result string when no assistant text is emitted", async () => {
  const { fn } = fakeQuery([{ type: "result", result: '{"text":"from-result"}' }]);
  const out = await new Enricher(undefined, fn).enrich({ text: "x", candidates: [] });
...
test("run skips non-text content blocks", async () => {
  const { fn } = fakeQuery([
    { type: "assistant", message: { content: [{ type: "thinking" }, { type: "text", text: '{"text":"kept"}' }] } },
  ]);
  const out = await new Enricher(undefined, fn).enrich({ text: "x", candidates: [] });
...
test("run passes the model to the SDK only when one is set", async () => {
  ...
  await new Enricher("claude-x", withModel.fn).enrich({ text: "x", candidates: [] });
  ...
  await new Enricher(undefined, noModel.fn).enrich({ text: "x", candidates: [] });
...
test("describeImage returns a trimmed caption", async () => {
  ...
  const out = await new Enricher(undefined, fn).describeImage(new Uint8Array([1, 2]), "image/png");
...
test("editText keeps the current text when the edit comes back empty", async () => {
  ...
  const out = await new Enricher(undefined, fn).editText("keep me", "do nothing");
